...
- **Supply-chain hygiene.** `npm run audit` (`npm audit --audit-level=high`) runs on every push alongside typecheck/lint/unit, and Dependabot opens weekly PRs for outdated npm and GitHub Actions dependencies — the same baseline a production suite would carry, not just a one-time clean install.
...
| `npm run audit`         | `npm audit --audit-level=high` — fails on new high/critical CVEs    |

CI runs typecheck + lint + unit tests + `npm audit` on every push, with Dependabot (`.github/dependabot.yml`) opening weekly PRs for outdated npm and GitHub Actions dependencies; the full suite is manual-dispatch only so CI never hammers a live production site.
...
The threshold is set at the current measured baseline, not an aspirational number: all five files now carry dedicated Vitest specs, including `sortAnalysis.ts`'s cross-source reconciliation functions (`reconcileRecencyOrder`, `formatReconciliation`, `formatViolation`, `formatViolationReport`), `structureAnalysis.ts`'s `analyzeListStructure`/`formatStructureIssues`, and `a11yAnalysis.ts`'s nested-target, missing-metadata, and plural-wording branches. The global threshold is 100%/100%/100%/100% (statements/branches/functions/lines) and the suite meets it exactly — `reconcileRecencyOrder` used to carry one unreachable defensive branch (`if (!bFirst || !bSecond) continue`), removed by restructuring the shared-id lookup so both sides come from a single filtered pairing instead of two independent `Map` gets that could never both fail (see SESSION_LOG.md).
