...
The threshold is set at the current measured baseline, not an aspirational number: all five files now carry dedicated Vitest specs, including `sortAnalysis.ts`'s cross-source reconciliation functions (`reconcileRecencyOrder`, `formatReconciliation`, `formatViolation`, `formatViolationReport`) and `structureAnalysis.ts`'s `analyzeListStructure`/`formatStructureIssues`, which were previously exercised only end-to-end by `tests/list-pages.spec.ts` and `tests/api.spec.ts`. The global baseline sits at 99%/94%/100%/100% (statements/branches/functions/lines); the two remaining uncovered branches are a genuinely unreachable defensive guard in `reconcileRecencyOrder` and pre-existing `a11yAnalysis.ts` branches out of this fix's scope, both left as-is rather than chased with contrived tests.
